...
namespace EWT
{
    public partial class EWTCheckmansForm : BaseForm
    {
        public EWTCheckmansForm()
        {
            InitializeComponent();
            
            ewtCheckmansDataGridView.AutoGenerateColumns = false;
        }
        ...
        public async Task LoadData()
        {
            ...
            {
                var reader = await connection.ExecuteReaderAsync(sql: "EWTsCheckMans_GetAll", commandType: CommandType.StoredProcedure);
                DataTable dataTable = new DataTable();
                dataTable.Load(reader);

                ewtCheckmansDataGridView.DataSource = dataTable;
            }

            MyErrorProvider.Clear();
        }

        private async void EWTCheckmansForm_Load(object sender, EventArgs e)
        {
            searchBox2.cbxSearchType.SelectedIndex = 1;
            ...
            using (IDbConnection connection = new SqlConnection(Utilities.Tools.GetEWTDBConnection))
            {
                var reader = await connection.ExecuteReaderAsync(sql: "SELECT u.[Pers_No],u.[FirstName],u.[LastName] FROM [EWTDB2].[dbo].[Users] u inner join [EWTDB2].[dbo].[UserRoles] ur on u.Id=ur.[User_Id] where  ur.Role_Id=1 and u.IsActive=1");
                DataTable dataTable = new DataTable();
                dataTable.Load(reader);
        ...
        private async Task<Models.EWTCheckMan> GetSingleEWTCheckMan(int EWT_Id, string PERS_NO)
        {
            using (IDbConnection connection = new SqlConnection(Utilities.Tools.GetEWTDBConnection))
            {
                Models.EWTCheckMan ewtCheckMan = null;
                DynamicParameters parameters = new DynamicParameters();
                parameters.Add("@EWT_Id", EWT_Id);
                parameters.Add("@PERS_NO", PERS_NO);

                ewtCheckMan =
                    await connection.QueryFirstOrDefaultAsync<Models.EWTCheckMan>("Select * From EWTCheckMans Where EWT_Id=@EWT_Id and PERS_NO=PERS_NO and IsActive=1", parameters);  //if checkman had a problem check this
        ...
        private async void ewtCheckmansDataGridView_SelectionChanged(object sender, EventArgs e)
        {
            dataEntryGroupBox.Enabled = false;

            if (ewtCheckmansDataGridView.CurrentRow == null)
            {
                return;
            }

            int ewt_Id = System.Convert.ToInt32(ewtCheckmansDataGridView.CurrentRow.Cells["EWT_Id"].Value);
            string pers_No = ewtCheckmansDataGridView.CurrentRow.Cells["PERS_NO"].Value.ToString();
            Models.EWTCheckMan ewtCheckMan = await GetSingleEWTCheckMan(ewt_Id, pers_No);

            if (ewtCheckMan != null)
            {
                ewtDataPicker.SelectedId = ewtCheckMan.EWT_Id;
                persDataPicker.SelectedId = System.Convert.ToInt32(ewtCheckMan.PERS_NO);
            }
            ...
            switch (FormDataMode)
            {
                case Enums.FormDataModeEnum.Add:
                    using (IDbConnection connection = new SqlConnection(Utilities.Tools.GetEWTDBConnection))
                    {
                        ...
                        await connection.ExecuteAsync("EWTCheckMan_Ins", param: parameters, commandType: CommandType.StoredProcedure);
                ...
                case Enums.FormDataModeEnum.Delete:
                    using (IDbConnection connection = new SqlConnection(Utilities.Tools.GetEWTDBConnection))
                    {
                        ...
                        connection.Execute("EWTCheckMan_Del", param: parameters, commandType: CommandType.StoredProcedure);
                    }
                    break;
            }
        ...
        private void tsbNew_Click(object sender, EventArgs e)
        {
            ...
            ClearForm();
            BindPersDataPicker("select id,Title from ewt where id not in (select ewt_Id from EWTCheckMans where IsActive=1)", CommandType.Text);
        }
